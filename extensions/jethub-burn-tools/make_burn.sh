#!/usr/bin/env bash
# make_burn.sh — convert Armbian .img to Amlogic burn image (JetHub J100, J80)
...
if [[ $# -ne 2 ]]; then
  display_alert "make_burn.sh" "Usage: $0 <input.img> j100" "error"
  exit 1
fi
...
if [[ "$BOARD" != "j100" ]]; then
  display_alert "make_burn.sh" "Only 'j100' is supported, got '$BOARD'" "error"
  exit 1
fi
...
display_alert "make_burn.sh" "Creating burn image for j100" "info"
